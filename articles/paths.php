<?php $title="Link Paths and PHP";
	...
	<div class="pageTitle">Paths and PHP</div>

	<h3>Links</h3>

	<p>Within a website, links work differently depending on where they are on that site and the different paths used to access them. Given the file structure below, I will explore some of the different ways in which links work.</p>
	personal_website/<br>
	- index.php<br>
	- goals.php<br>
	- dog.php<br>
	...
	<p>Numbers one through three are all known as relative paths. These paths will all take you somewhere relative to where you are currently located. From index, number one will take you to straight to the goals page, as it is located in the same folder. Number two has a forward slash &quot;/&quot;, which refers to the root directory in this case. Three is used to move up a folder. In the case of moving from the index, there is nowhere else to go. The fourth link is an absolute link, which you can tell by the &quot;http://&quot;. No matter what page you are on, number four will always take you to the goals.php through the local server.</p>
	...
	<p>These are all still relative links. Number one will include navigation.php as they are in the same folder, meaning they are relative to each other. Number two . Number three will move up a folder from index. Since navigation.php is not located there, the file won't work since it doesn't exist in that context.</p>
	<p>Navigation.php is also accessed through articles.hacking.php. In this situation, number one would not work as it would be searching for navigation.php relative to hacking within the articles subfolder. Number two . Number three would however work as intended, with the navigation page functioning properly since &quot;../&quot; allowed us to go back from the subfolder.
	Localhost vs server
	</p>
